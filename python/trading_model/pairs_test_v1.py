...
def main():
    path = 'C:\\Users\\YChen\\Documents\\git\\working_dir\\python\\data\\Cointegration_inout_v0.csv'
    ...
    v_spread = np.diff(prices,axis=0)
    ...
    if m < numOfCol:
        ...
        while j <= numOfCol:
            ...
            if j <= numOfCol-n:
                ...
                v_Beta[:], v_ADF[:], v_h[:] = fill_value(a,b,adf2,cv,v_h,v_Beta,v_ADF,j,numOfCol,0)
            # beta index
            if j <= numOfCol-n:
                v_residual, zscr = fill_residual_zscr(v_residual,zscr,resid,beta_idx,prices,a,b,stderr,j,0,n,m)
            else:
                v_residual, zscr = fill_residual_zscr(v_residual,zscr,resid,beta_idx,prices,a,b,stderr,j,-1,0,m)
            j += n
    else:
        b, a, adf2, cv, stderr = modeling(prices[1,:],prices[0,:]) #second row is x, first row is y
    ...
    ## part 2
    cross = 0
    v_cross = np.zeros((numOfCol,1)) 
    for ctr in range(0,numOfCol-1):
    ...
    exp_ret, exp_vol, winp, omega, exp_hp, trades = trade_decision(signal_TH,hp_TH,prices,m,numOfCol,adj_day,zscr,v_h)
    #save the result
    result = [exp_ret,exp_vol,winp,omega,exp_hp,trades,v_Beta.item(-1),v_ADF.item(-1)]
    saveFile('C:\\Users\\YChen\\Documents\\git\\working_dir\\python\\data\\Cointegration_output_v1.csv',result)
