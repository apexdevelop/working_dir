...
def coint_datafeed(path,field,startdate,enddate,period,curr):
    try:
        ...
        l_px2=[]
        sid1 = mgr[arr1]
        df_px1 = sid1.get_historical(field, startdate, enddate,period, currency=curr)
        ...
        sid2 = mgr[arr2]
        df_px2 = sid2.get_historical(field, startdate, enddate,period, currency=curr)
...
def align_data(l_px1,l_px2,numPairs):
    l_aligned=[]
...
def modeling(x,y):
    ...
    return beta, alpha, teststat, p_value,stderr, residual

def fill_value(alpha,beta,adf2,pv,cv,v_h,v_Beta,v_ADF,v_pADF,start,end,window):
    if window != 0: 
        for i in range(start, start + window):
            v_Beta[start-1:start+window-1] = np.ones((window,1))*beta
            v_ADF[start-1:start+window-1] = np.ones((window,1))*adf2
            v_pADF[start-1:start+window-1] = np.ones((window,1))*pv
            if adf2 > cv: 
        ...
        for i in range(start,end + 1):
            ...
            v_pADF[start-1:] = np.ones((end-start+1,1))*pv
            if adf2 > cv: 
                v_h[start-1:] = np.zeros((end-start+1,1))
            else:
                v_h[start-1:] = np.ones((end-start+1,1))
    return v_Beta, v_ADF, v_pADF, v_h
...
def main():
    ...
    period='DAILY'
    curr='USD'
    raw_lpx1,raw_lpx2 = coint_datafeed(path,field,startdate,enddate,period,curr)
    numPairs=len(raw_lpx1)
    l_aligned=align_data(raw_lpx1,raw_lpx2,numPairs)
    ...
    for i in range(numPairs):
        ...
        if m < numOfOb:
           ...
           while j <= numOfOb:
                 ...
                 b, a, adf2, pv,stderr, resid = modeling(ny1[1,:],ny1[0,:]) #second row is x, first row is y
                 if j <= numOfOb-n:
                    v_Beta[:], v_ADF[:], v_pADF[:], v_h[:] = fill_value(a,b,adf2,pv,cv,v_h,v_Beta,v_ADF,v_pADF,j,0,n)
                    v_residual, zscr = fill_residual_zscr(v_residual,zscr,resid,beta_idx,row_logpx,a,b,stderr,j,0,n,m)
                 else:
                    v_Beta[:], v_ADF[:], v_pADF[:], v_h[:] = fill_value(a,b,adf2,pv,cv,v_h,v_Beta,v_ADF,v_pADF,j,numOfOb,0)
                    v_residual, zscr = fill_residual_zscr(v_residual,zscr,resid,beta_idx,row_logpx,a,b,stderr,j,-1,0,m) 
                 j += n
        else:
           b, a, adf2, pv, stderr = modeling(row_logpx[1,:],row_logpx[0,:]) #second row is x, first row is y
        ...
        #optimize for best return
        v_exp_ret=np.zeros((5,1))
        l_result_perpair=[]
        for j in range(5):
            signal_TH=1+0.5*j
            exp_ret, exp_vol, winp, omega, exp_hp, trades = trade_decision(signal_TH,hp_TH,row_logpx,m,numOfOb,adj_day,zscr,v_h)
            result = [exp_ret,exp_vol,winp,omega,exp_hp,trades]
            l_result_perpair.append(result)
            v_exp_ret[j]=exp_ret
        max_idx=np.argmax(v_exp_ret)
        opti_zSignal=1+max_idx*0.5
        opti_ret=np.amax(v_exp_ret)
        corres_list=l_result_perpair[max_idx]
        corres_list.insert(0,opti_zSignal)
        #opti_result = [exp_ret,exp_vol,winp,omega,exp_hp,trades,v_Beta.item(-1),v_ADF.item(-1)]
        l_result_multi.append(corres_list)
    #save the result
    saveFile('C:\\Users\\YChen\\Documents\\git\\working_dir\\python\\data\\Cointegration_output_v4.csv',l_result_multi)
