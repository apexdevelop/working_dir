...
def main():
    ...
    #save the result
    raw = np.concatenate((raw[0,None].T,prices[0:].T,v_residual,zscr,v_ADF,v_pADF,v_h,v_Beta),axis=1)
    #raw = np.vstack((raw[0,:].T,prices[0:,:].T),axis=1)
    saveFile('C:\\Users\\YChen\\Documents\\git\\working_dir\\python\\Cointegration_output_v0.csv',raw)
