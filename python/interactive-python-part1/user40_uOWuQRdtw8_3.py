...
def expected_value(held_dice, num_die_sides, num_free_dice):
    ...
    exp_value=sum(value_list)/len(value_list)
    return exp_value
...
def gen_all_holds(hand):
    ...
    for idx in range(len(hand)):       
        for partial_seq in unsorted_set:
            list_partial_seq=list(partial_seq)
            list_hand=list(hand)
            for each in list_partial_seq:
                new_idx=list_hand.index(each)
                list_hand.pop(new_idx)
            for entry in list_hand:                
                new_seq=list(partial_seq)
                new_seq.append(entry)
                temp_set.add(tuple(new_seq))
        unsorted_set=temp_set
    sorted_list = [tuple(sorted(seq)) for seq in unsorted_set]
    sorted_set=set(sorted_list)
    sorted_set.add(tuple())
    return sorted_set
