...
def load_yahoo_stock(sids, start=None, end=None, dvds=True):
    if hasattr(sids, '__iter__') and not isinstance(sids, basestring):
        ...
        data_yahoo = data.DataReader(sid, 'yahoo',start=start, end=end)
        data_yahoo = data_yahoo.rename(columns=lambda c: c.lower())
        if dvds:
            ...
            if not d.empty:
                ...
                data_yahoo = data_yahoo.join(d)
            else:
                data_yahoo['dvds'] = np.nan
        pxs = InstrumentPrices(data_yahoo)
