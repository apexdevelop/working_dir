...
for (i in 12:12) { 
  ...
  df.clean.date=df.date[!is.na(df.ret)]
  name.ret=name.list[i-1]
  
  mtx.date=as.matrix(df.clean.date[2:63])
  ...
  bench.clean.xts=cbind(bench.xts[!is.na(df.ret),1],bench.xts[!is.na(df.ret),2])
...
write.csv(temp.metrics.5, file.dir.out,row.names = F,quote = F )
