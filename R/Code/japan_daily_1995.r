...
factor2=f2[3708:(num_obs_f-f_ind2),]
...
factor4=f4[2832:(num_obs_f-f_ind4),]

f5=raw_factor[,9:10]
f_ind5=0
for (i in 1:num_obs_f){
    if (is.na(f5[i,2])==TRUE) f_ind5=f_ind5+1}
factor5=f5[4579:(num_obs_f-f_ind5),]

f6=raw_factor[,11:12]
f_ind6=0
for (i in 1:num_obs_f){
    if (is.na(f6[i,2])==TRUE) f_ind6=f_ind6+1}
factor6=f6[4603:(num_obs_f-f_ind6),]

f7=raw_factor[,13:14]
f_ind7=0
for (i in 1:num_obs_f){
    if (is.na(f7[i,2])==TRUE) f_ind7=f_ind7+1}
factor7=f7[4581:(num_obs_f-f_ind7),]
...
factor9=f9[4676:(num_obs_f-f_ind9),]

f10=raw_factor[,19:20]
f_ind10=0
for (i in 1:num_obs_f){
    if (is.na(f10[i,2])==TRUE) f_ind10=f_ind10+1}
factor10=f10[4410:(num_obs_f-f_ind10),]
...
tmp_f1.zoo=merge(tmp1.zoo,tmp4.zoo,all=FALSE)
