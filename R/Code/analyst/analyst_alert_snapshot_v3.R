...
for (i in 1:num_ob) {
  ...
  ##previouly, use customized end date assume field data updates timely
...
#output = cbind(out_label_cutoff,out_event,col_date,out_broker,col_ticker,out_name,is_forced,out_oldtp,out_newtp2,out_newtp,out_newtp_px,out_oldtp_px,chg_tgt,out_newtp_avgtp,re_chg2d,re_chg5d,out_newrating,out_newrating_numeric,out_oldrating_numeric,chg_rating,out_score)
...
#colnames(df.output)=c("time","event","date","broker","ticker","name","is_forced","oldtp","newtp_datafeed","newtp_snapshot","newtp_to_px","oldtp_to_px","chg_tgt","newtp_avgtp","chg_2d","chg_5d","char_newrating","newrating","oldrating","chg_rating","score")
