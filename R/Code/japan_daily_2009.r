...
raw_factor=read.csv("japan_multifactor_return.csv", stringsAsFactors=F)
...
factor1=f1[startpoint1:893,]

f2=raw_factor[,3:4]
f_ind2=0
for (i in 1:num_obs_f){
    if (is.na(f2[i,2])==TRUE) f_ind2=f_ind2+1}
factor2=f2[1:(num_obs_f-893),]

f3=raw_factor[,5:6]
f_ind3=0
for (i in 1:num_obs_f){
    if (is.na(f3[i,2])==TRUE) f_ind3=f_ind3+1}
factor3=f3[startpoint1:893,]
...
tmp1.zoo<-merge(factor5n6.zoo,factor5n1.zoo,all=FALSE)
tmp2.zoo<-merge(factor8n4.zoo,factor7n8.zoo,all=FALSE)
tmp3.zoo=merge(tmp1.zoo,tmp2.zoo,all=FALSE)
tmp4.zoo=merge(factor9.zoo,factor10.zoo)

tmp_f.zoo=merge(tmp3.zoo,tmp4.zoo,all=FALSE)
tmp_f1.zoo=merge(tmp3.zoo,tmp4.zoo,all=FALSE)
...
lm.0=lm(t[11:776,1]~t[10:775,6]+t[11:776,3]+t[11:776,2]+t[10:775,4]+t[11:776,5]+t[11:776,7])
#lm.1=lm(t1[11:157,1]~t1[7:153,2]+t1[11:157,3]+t1[10:156,4]+t1[11:157,5])
#lm.1=lm(t1[11:157,1]~t1[7:153,2]+t1[11:157,3]+t1[10:156,4]+t1[11:157,5]+t1[11:157,5]:t1[7:153,2]+t1[11:157,5]:t1[10:156,4]++t1[11:157,5]:t1[11:157,3])
