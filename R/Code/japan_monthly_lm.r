...
raw_factor=read.csv("japan_multifactor_monthly_return.csv", stringsAsFactors=F)
...
tmp1.zoo<-merge(factor5.zoo,factor6.zoo,all=FALSE)
m1.zoo=tmp1.zoo$factor5.zoo-tmp1.zoo$factor6.zoo
tmp2.zoo<-merge(factor5.zoo,factor1.zoo,all=FALSE)
m2.zoo=tmp2.zoo$factor5.zoo-tmp2.zoo$factor1.zoo
tmp3.zoo<-merge(factor9.zoo,factor10.zoo,all=FALSE)
m3.zoo=tmp3.zoo$factor10.zoo-tmp3.zoo$factor9.zoo

tmp3.zoo<-merge(m1.zoo,m2.zoo,all=FALSE)
tmp4.zoo<-merge(m3.zoo,factor11.zoo,all=FALSE)
...
ind1_p=0
for(i in 2:(num_obs-ind1)){
    if(price1[i,2]=="") ind1_p=ind1_p+1}
price1=price1[2:(num_obs-ind1-ind1_p),]
