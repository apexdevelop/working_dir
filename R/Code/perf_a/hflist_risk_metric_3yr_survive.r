...
df.metrics.3=NULL  #calculated manually
...
for (i in 1:n.sec) {
  ...
  if (nrows>=th.nrows && nrows.2017>=th.sur){
    ...
    df.metrics.3=rbind(df.metrics.3,temp.metrics.5)
    
  }
}

outfile=paste(u.name,"_rcustom_3yr.csv",sep="")
file.dir.out=paste(pathname,outfile,sep="")
#write.csv(df.metrics.3, file.dir.out)
library(xlsx)
xlsx.dir.out=paste(pathname,curmon.chr,"_3yr_output.xlsx",sep="")
xlsx.sheetname=paste(u.name,"_rcustom_3yr",sep="")
#has to be new worksheet
write.xlsx(df.metrics.3,xlsx.dir.out,sheetName=xlsx.sheetname,append=TRUE,showNA=FALSE)
