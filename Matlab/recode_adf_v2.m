...
%1 + number of estimated parameters in cointegrating vector, excluding deterministic terms (1:12)     
tableDim = 1 + 1; 
...
testT = N-(testLags+1);   % Effective sample size
...
testReg.size = testT;
testReg.names = names;
testReg.coeff = coeff2;
testReg.se = sqrt(diag(Cov));
testReg.Cov = Cov2;
testReg.tStats.t = coeff2./testReg.se;
...
testReg.LL = -normlike([0,sqrt(MSE)],res2);
