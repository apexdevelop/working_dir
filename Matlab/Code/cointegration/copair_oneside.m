function [pairoutput,v_pADF,zscr]=copair_oneside(method,rtn_Y,Y,M,N,Signal_TH,pADF_TH,hp_TH,tday)
...
%    pADF_TH=pValue;
   for j=M:size(Y,1)
         if is_open~=0 %that means position opened  
            ...
            r_trade(entryIDX)=0;
            current_r=[0;s(entryIDX:j-1, :) .* rtn_Y(entryIDX:j-1,1)];
            r_trade(j)= sum(current_r);
            cumul_r= [0 ; s(1:j-1, :) .* rtn_Y(1:j-1,1)];
            CPnL(j)= sum(cumul_r);
            
            %exit
            if (s(j,1)==-1 && zscr(j)<-1) || (s(j,1)==1 && zscr(j)>1) ...
