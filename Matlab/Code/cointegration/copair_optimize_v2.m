...
[~,txt1]=xlsread('Copair_crossborder_8.xlsm','pairs','e2:e102'); %last line 154
[~,txt2]=xlsread('Copair_crossborder_8.xlsm','pairs','h2:h102');
...
for sn=1:n_pair
    ...
    [max_idx,respmax1,param1,resp,V_var] = parameterSweep_v2(pfun1,range1,opti_idx);
    toc
...
%        end
%     end
    c_para=num2cell(V_var);
    n_combination=size(c_para,1);
    c_names={n_combination,2};
    for i=1:n_combination
        c_names{i,1}=txt1{sn};
        c_names{i,2}=txt2{sn};
    end
    c_StockWise{1,sn}=[c_names,c_para,num2cell(resp)];
    c_newopti=[txt1{sn},txt2{sn},num2cell(param1),num2cell(resp(max_idx,:))];
    c_OPTI=[c_OPTI;c_newopti];
    c_big_metrics=[c_big_metrics;[c_names,c_para,num2cell(resp)]];
end
cd('C:/Users/YChen/Documents/git/working_dir/Matlab/Data/Coint');
save (strcat('c_crossborder_OPTI_betaidx_',num2str(beta_idx),'_isadf_',is_adf), 'c_OPTI');
save (strcat('c_crossborder_StockWise_betaidx_',num2str(beta_idx),'_isadf_',is_adf), 'c_StockWise');