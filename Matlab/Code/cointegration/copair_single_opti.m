...
% txt1={'2409 TT Equity';'TWSE Index';'034220 KS Equity';'KOSPI Index'};
% txt2={'2009/01/01';2};
...
function [tday_excel,log_e1,log_e2,v_Alpha,v_Beta,v_Beta2,v_spread,z_spread,v_residual,z_residual,v_pADF,signal_long,signal_short,signal_exit,r,CPnL,bimat,metric]=copair_single_opti(txt1,txt2)
enddate=today();
...
rel_logY=extract_country(logY1,logY2,4);
...
% combination of calibration window and rebalancing frequency.
   ...
   pfun1 = @(x) pairsFun(is_adf, beta_idx, x, rel_logY, scaling, cost);
           
   tic
   [respmax1,param1,resp1] = parameterSweep(pfun1,range1);
   toc
   
   bimat=zeros(size(spread,2),size(p_ADF,2));
   ...
   Signal_TH=param1(3);
   pADF_TH=param1(4);
   [metric,v_Alpha,v_Beta,v_Beta2,v_spread,z_spread,v_residual,z_residual,v_pADF,signal_long,signal_short,signal_exit,r,CPnL]=copair_tsoutput1(is_adf,beta_idx,rel_logY,param1(1),param1(2),Signal_TH,pADF_TH,hp_TH,scaling,cost,tday);
   metric=[metric,param1];
   tday_excel=m2xdate(tday(param1(1):end),0);
   log_e1=rel_logY(param1(1):end,1);
   log_e2=rel_logY(param1(1):end,2);
