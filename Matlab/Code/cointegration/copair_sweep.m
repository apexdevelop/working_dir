function [pairoutput,r]=copair_sweep(beta_idx,Y,M,N,Signal_TH,pADF_TH,hp_TH,scaling,cost,tday,min_spread,max_pADF,biret,signal_step,status)
%% Process input args
...
%calculate long term pADF and Beta
% if size(Y,1)>=500
%    [~,ltp,~,~,ltreg] = egcitest(Y(end-500:end,:),'alpha',pADF_TH);
% else
%    [~,ltp,~,~,ltreg] = egcitest(Y,'alpha',pADF_TH);
% end
% m_Beta=ltreg.coeff(2);
% m_pADF=ltp;
...
if max_signal>=1 && max_signal<=2.5
   exp_ret=biret(signal_idx,2);
else
   exp_ret=-100;
end 
across_ret=mean(biret);
...
%    pADF_TH=pValue;
   for j=M:size(Y,1)
         if is_open~=0 %that means position opened  
            ...
            CPnL(j)= CPnL(j-1);
            if  s(j-1,1)==0 && zscr(j-2)>=Signal_TH && zscr(j-1)>0.5*Signal_TH && zscr(j)>0.5*Signal_TH ...
                    && (zscr(j)<=zscr(j-2) || zscr(j)<=zscr(j-1)) ...
                && v_pADF(j)<=pADF_TH ...
%                 && z_spread(j)>0
                s(j, 1) = -1;
                for col=2:size(Y,2)
                ...
                Enter=[Enter;j];
                Short=[Short;j];
            elseif s(j-1,1)==0 && zscr(j-2)<=-Signal_TH && zscr(j-1)<-0.5*Signal_TH && zscr(j)<-0.5*Signal_TH ...
                    && (zscr(j)>=zscr(j-2) || zscr(j)>=zscr(j-1)) ...
                && v_pADF(j)<=pADF_TH ...
%                 && z_spread(j)<0
                s(j, 1) = 1;
...
if size(Y,2)==2
   pairoutput=[Signal_TH, pValue,last_zspread,signal_l2,signal_l,signal_c,av_ret,winp,reg1_rtn.coeff(2),reg1_log.RSq,...
   hp,avg_cross_elapsed,yr_trades,yr_cross,is_open,direction,last_exit,last_enter,last_elapsed,last_distance,last_extreme,exp_ret,across_ret,mean(v_pADF)];
else
   pairoutput=[reg1_rtn.coeff(2),reg1_rtn.coeff(3),reg1_log.coeff(2),reg1_log.coeff(3),reg1_log.RSq,pValue,signal_l2,signal_l,signal_c,...
   av_ret,hp,avg_cross_elapsed,yr_trades,yr_cross,winp,last_exit,last_enter,last_elapsed,last_distance,last_extreme];
end
