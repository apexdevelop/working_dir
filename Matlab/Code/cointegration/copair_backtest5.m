%updated signal_l2, signal_l, signal_c when trade ends

function [pairoutput,r,zscr,r_trade]=copair_backtest5(beta_idx,Y,M,N,Signal_TH,pADF_TH,hp_TH,scaling,cost,tday,min_spread,max_pADF,status)
%% Process input args
...
%             v_plus=find(zscr((j-3*trade_window-1):(j-trade_window-1))>=-0.05*Signal_TH);
            if  s(j-1,1)==0 && zscr(j)<Signal_TH && zscr(j-1)>=Signal_TH && zscr(j)>=0.5*Signal_TH  ...
                && v_pADF(j)<=0.9*max_pADF ...
%                 && z_spread(j)>0
                s(j, 1) = -1;
                for col=2:size(Y,2)
                    s(j,col) = - s(j, 1);
                ...
                signal_l=zscr(end-1);
                signal_c=zscr(end);
            elseif s(j-1,1)==0 && zscr(j)>-Signal_TH && zscr(j-1)<=-Signal_TH && zscr(j)<=-0.5*Signal_TH ...
                && v_pADF(j)<=0.9*max_pADF ...
%                 && z_spread(j)<0
                s(j, 1) = 1;
                for col=2:size(Y,2)
...
pairoutput=[pADF_TH, pValue,last_zspread,signal_l2,signal_l,signal_c,Signal_TH,av_ret,winp,reg1_rtn.coeff(2),reg1_log.RSq,...
   hp,avg_cross_elapsed,yr_trades,yr_cross,is_open,direction,last_exit,last_enter,last_elapsed,last_distance,last_extreme,lt_pADF,signal_enter,omega];
