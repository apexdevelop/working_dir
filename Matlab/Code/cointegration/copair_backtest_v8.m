...
function [pairoutput,zscr]=copair_backtest_v8(c_Stock,beta_idx,Y,M,N,old_Signal_TH,old_pADF_TH,hp_TH,scaling,cost,tday_m,old_status,direction,old_ret,old_vol,old_winp,old_omega,old_hp,old_trades,old_enterdate_m,old_exitdate_m)
%% Process input args
...
if M< size(Y,1)
   ...
   while j<=size(Y,1)
      ...
      if j<=size(Y,1)-N
         ...
         if j==M
            v_residual(j-M+1:j)=reg1_log.res;
         end
