% mean(v_pADF) and lt_pADF
% record enter pADF
function [pairoutput,r]=copair_backtest4(beta_idx,Y,M,N,Signal_TH,pADF_TH,hp_TH,scaling,cost,tday,min_spread,max_pADF,status)
%% Process input args
...
%                 || v_pADF(j)>max_pADF
            ...
            CPnL(j)= CPnL(j-1);
            if  s(j-1,1)==0 && zscr(j-2)>=0.5*Signal_TH && zscr(j-1)>0.40*Signal_TH && zscr(j)>0.40*Signal_TH ...
...
%                 && z_spread(j)>0
                ...
                Enter=[Enter;j];
                Short=[Short;j];
            elseif s(j-1,1)==0 && zscr(j-2)<=-0.5*Signal_TH && zscr(j-1)<-0.40*Signal_TH && zscr(j)<-0.40*Signal_TH ...
                    && (zscr(j)>=zscr(j-2) || zscr(j)>=zscr(j-1)) ...
%                 && v_pADF(j)<=pADF_TH ...
%                 && z_spread(j)<0
                s(j, 1) = 1;
