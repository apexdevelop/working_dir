function [pairoutput,r,zscr,r_trade]=copair_backtest6(beta_idx,Y,M,N,Signal_TH,pADF_TH,hp_TH,scaling,cost,tday,min_spread,max_pADF,status)
%% Process input args
...
if Signal_TH>=min_spread && lt_pADF<=max_pADF
   for j=M:size(Y,1)
         if is_open~=0 %that means position opened  
            ...
            CPnL(j)= CPnL(j-1);
%             v_minus=find(zscr((j-3*trade_window-1):(j-trade_window-1))<=0.05*Signal_TH);
%             v_plus=find(zscr((j-3*trade_window-1):(j-trade_window-1))>=-0.05*Signal_TH);
            if  s(j-1,1)==0 && zscr(j-1)>Signal_TH && zscr(j)<=Signal_TH && zscr(j)>=0.5*Signal_TH && mean(zscr((j-3*trade_window-1):(j-trade_window-1)))<=0.05*Signal_TH ...
                && v_pADF(j)<=max_pADF ...
%                 && z_spread(j)>0
                s(j, 1) = -1;
                for col=2:size(Y,2)
                ...
                Enter=[Enter;j];
                Short=[Short;j];
            elseif s(j-1,1)==0 && zscr(j-1)<-Signal_TH && zscr(j)>=-Signal_TH && zscr(j)<=-0.5*Signal_TH && mean(zscr((j-3*trade_window-1):(j-trade_window-1)))>=-0.05*Signal_TH ...
                && v_pADF(j)<=max_pADF ...
%                 && z_spread(j)<0
                s(j, 1) = 1;
                for col=2:size(Y,2)
...
pairoutput=[last_zspread,signal_l2,signal_l,signal_c,Signal_TH,pADF_TH,is_open,direction,av_ret,av_vol,winp,omega,hp,trades,last_enter,last_exit,pValue,lt_pADF,beta,R2,...
