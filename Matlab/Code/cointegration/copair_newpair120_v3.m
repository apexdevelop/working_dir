...
% end_idx=159;
% 
% [~,txt1]=xlsread('Copair_samecountry_9.xlsm','pairs',strcat('e',num2str(start_idx),':e',num2str(end_idx)));
...
function [output_Metrics,filtered2_stk1,filtered2_stk2,filtered2_M,filtered1_stk1,filtered1_stk2,filtered1_M,v_Rtn]= copair_newpair120_v3(txt1,txt2,input_param,input_Metrics)
...
% M =120;
M = cell2mat(input_param(5));
N = cell2mat(input_param(6));
scaling=1;
cost=0;
hp_TH=44;
...
S_StockWise=load(strcat('c_StockWise_betaidx_',num2str(beta_idx),'_isadf_',is_adf));
c_StockWise=S_StockWise.c_StockWise;

nameCell=[txt1 txt2];
%after converting to string,quotes were removed when pasting to excel
...
for sn=1:n_pair
    ...
    old_enterdate_m=x2mdate(mat_enterdates(sn));
    old_exitdate_m=x2mdate(mat_exitdates(sn));
    %%
...
%        TH_idx=find(thLia);
       ...
       [new_metric,zscr]=copair_backtest_v8(c_Stock,beta_idx,Y,M,N,old_Signal_TH,old_pADF_TH,hp_TH,scaling,cost,tday_m,old_status,direction,old_ret,old_vol,old_winp,old_omega,old_hp,old_trades,old_enterdate_m,old_exitdate_m);
       is_database=1;
    ...
    %% filtering
    %filtering criteria1 
    %abs(Beta)<3 and abs(Beta)>0.2, long term pADF<=max_pADF,is_open=1,has
