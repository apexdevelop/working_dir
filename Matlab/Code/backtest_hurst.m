% line 31, line 64, only calculate magnitude of return, ignore positive or
% negative return
function [s, output]=backtest_hurst(v_rtn,Y,v_hurst,enter_disp,exit_disp,tday)
...
direction=0;
hp_TH=40;
s = zeros(size(v_rtn,1),size(v_rtn,2));
...
CPnL=zeros(size(Y,1),1);
     ...
     for j=3:size(Y,1)
         if is_open~=0 %that means position opened  
            ...
            %exit
            if Y(j)<exit_disp || v_hurst(j)<0.45|| r_trade(j)<-15 || j-entryIDX > hp_TH
                trades=trades+1;
            ...
            CPnL(j)= CPnL(j-1);
            if  Y(j)>= enter_disp && v_hurst(j)>=0.5 
                if v_rtn(j)>0
                   s(j, 1) = 1;
                   entryIDX=j;
                   is_open=1;
