...
function output=backtest(v_rtn,Y,enter_disp,exit_disp,v_corr,enter_corr,exit_corr,tday)
...
if enter_disp>0
     for j=2:size(Y,1)
         if is_open~=0 %that means position opened  
            ...
            %exit
            if (direction==1 && Y(j)<exit_disp) || v_corr(j) > exit_corr || r_trade(j)<-15 || j-entryIDX > hp_TH
                trades=trades+1;
...
output=[enter_disp,exit_disp,enter_corr,exit_corr,av_ret,stdev,yr_trades,hp,winp,last_exit,last_enter];