...
data_dir = strcat(home_dir,'/git/working_dir/Matlab/Data/factor/multifactor');
cd(data_dir);
% cd('C:/Users/YChen/Documents/git/working_dir/Matlab/Data/factor/multifactor');
...
d_ranges={'b6:b39','b6:b39','b6:b29','b6:b32','b6:b38','b6:b35','b6:b39','b6:b40','b6:b36','b6:b46','b6:b43','b6:b11','b6:b30','b6:b22','b6:b37','b6:b28','b6:b23'};
w_ranges={'b40:b46','b40:b47','b30:b30','b33:b33','b39:b42','b36:b36','b40:b41','b41:b42','b37:b42','b50:b50','b44:b44','b12:b12','b31:b44','b23:b38','b39:b39','b29:b29','b24:b29'};
...
predirection_ranges={'d18:g18','d18:n18','d18:h18','d18:d18','d18:i18','d18:f18','d18:g18','d18:d18','d18:i18','d18:p18','d18:g18','d18:d18','d18:e18','d18:g18'};
TH_ranges={'d3:g3','d3:n3','d3:h3','d3:d3','d3:i3','d3:f3','d3:g3','d3:d3','d3:h3','d3:p3','d3:g3','d3:d3','d3:e3','d3:g3'};
para_ranges={'d4:g14','d4:n14','d4:h14','d4:d14','d4:i14','d4:f14','d4:g14','d4:d14','d4:h14','d4:p14','d4:g14','d4:d14','d4:e14','d4:g14'};
exit_ranges={'d16:g16','d16:n16','d16:h16','d16:d16','d16:i16','d16:f16','d16:g16','d16:d16','d16:h16','d16:p16','d16:g16','d16:d16','d16:e16','d16:g16'};
enter_ranges={'d17:g17','d17:n17','d17:h17','d17:d17','d17:i17','d17:f17','d17:g17','d17:d17','d17:h17','d17:p17','d17:g17','d17:d17','d17:e17','d17:g17'};

beta_ranges={'d6:g35','d6:n47','d6:h29','d6:d32','d6:i42','d6:f35','d6:g41','d6:d42','d6:h43','d6:p45','d6:g44','d6:d11','d6:e44','d6:g38'};
fchg_ranges={'i6:m35','q6:u47','j6:n29','f6:j32','k6:o42','h6:l35','i6:m41','f6:j42','j6:n43','r6:v45','i6:m44','f6:j11','g6:k44','i6:m38'};
...
filename3='stepwise_output.xlsx';
filename4='stepwise_trade.xlsm';
...
[v_predirection,~]=xlsread(filename2,shname,'d18:zz18'); 
...
mat_inmodel=[];
mat_b=[];
mat_fnames_inmodel = cell(1,size(txt2,2));
mat_fchg_inmodel= cell(1,size(txt2,2));
mat_output= cell(1,size(txt2,2));
%% generate weekly factor data
[~,txt_fw]=xlsread(filename1,shname,char(w_ranges(sh_idx))); %weekly factor
txt_f=[txt1;txt_fw]; % combine daily and weekly factors
if ~isempty(txt_fw)
c=blp;
for loop=1:size(txt_fw,1)
    new=char(txt_fw(loop));
...
close(c);
re_rtns_5d=zeros(size(rtns_w5d,1),size(txt_fw,1));
re_fwpxs=zeros(size(fwpxs,1),size(txt_fw,1));
crtns_w5d=zeros(1,size(txt_fw,1));
z_rtns_w5d=zeros(1,size(txt_fw,1));
z_fpxs=zeros(1,size(txt_fw,1));
...
n_dim=size(txt1,1);
rtns_1d=zeros(1,n_dim+size(txt_fw,1));
z_rtns_1d=zeros(1,n_dim+size(txt_fw,1));
...
% for q=4:4
    ...
    while j<=size(rtn_Y,1)
          ...
          Y=rtn_Y(j-M+1:j,1);
          [b,se,pval,inmodel,stats,nextstep,s_history]=stepwisefit(X,Y,'penter',p_enter(sh_idx),'premove',p_exit(sh_idx),'display','off','inmodel',v_inmodel,'keep',v_keep);
          if sum(inmodel)==0
             step=0.1;
             while sum(inmodel)==0 && p_enter(sh_idx)+step<1 && p_exit(sh_idx)+step<1
                   [b,se,pval,inmodel,stats,nextstep,s_history]=stepwisefit(X,Y,'penter',p_enter(sh_idx)+step,'premove',p_exit(sh_idx)+step,'display','off','inmodel',v_inmodel,'keep',v_keep);   
                   step=step+0.1;
             end
    ...
    names_inmodel=[txt2(q);txt_f(inmodel)];
    %mat_fnames_inmodel{1,q}=fnames_inmodel;
...
%     inmodel=v_inmodel;
    ...
    while j<=size(rtn_Y,1)
          ...
          if j<=size(rtn_Y,1)-N
             ...
             mean_res_N=mean(v_residual(j-M+1:j-M+N)); 
%              std_res_N=std(v_residual(j-M+1:j-M+N));
%              mean_res_N=mean(reg.res(end-window:end)); %window is for zscore
%              std_res_N=std(reg.res(end-window:end)); %window is for zscore
%              zscr(j-M+1:j-M+N)=(v_residual(j-M+1:j-M+N)-mean_res_N)/std_res_N;
             zscr(j-M+1:j-M+N)=(v_residual(j-M+1:j-M+N)-mean_res_N)/reg.RMSE;         
          else
             log_XY2=log([Px_Y(j:end,1) Px(j:end,inmodel)]);
             v_residual(j-M+1:end)=log_XY2*coint_b-reg.coeff(1);
             zscr(j-M+1:end)=(v_residual(j-M+1:end)-mean_res_N)/reg.RMSE;
%              zscr(j-M+1:end)=(v_residual(j-M+1:end)-mean_res_N)/std_res_N;
          end
          j=j+N;
    end
    %build output
    log_XY3=log([Px_Y(end-N:end,1) Px(end-N:end,inmodel)]);
    chg_inmodel_N=100*(log_XY3(end,:)-log_XY3(1,:)); % chg in N days
    chg_inmodel_3=100*(log_XY3(end,:)-log_XY3(end-3,:)); % chg in 3 days
    
    logpx_inmodel=log([Px_Y(end,1) Px(end,inmodel)]);
    
    output_beta=[coint_b(1);-1*coint_b(2:end)];
    temp_cell=num2cell([output_beta transpose(chg_inmodel_N) transpose(chg_inmodel_3) transpose(logpx_inmodel)]);
    mat_output{1,q}=[names_inmodel temp_cell];
    ...
    [current_metric,s1,ret_v1,r_trade1]=z_backtest_oneside1(v_para(:,q),v_direction(q),v_predirection(q),rtn_eb(M:end,:),zscr,v_TH(q),hp_TH,tday1(M:end),v_exit(q),v_enter(q));    
...
c_predirection=transpose(current_Metrics(:,18));
c_para=transpose(current_Metrics(:,6:16));
c_exit=transpose(current_Metrics(:,19));
c_enter=transpose(current_Metrics(:,20));

cd(data_dir);
% cd('C:/Users/YChen/Documents/git/working_dir/Matlab/Data/factor/multifactor');
xlswrite(filename1,inmodel_b,shname,char(beta_ranges(sh_idx))); %in model beta
xlswrite(filename1,mat_fchg,shname,char(fchg_ranges(sh_idx))); %factor move
...
xlswrite(filename2,c_predirection,shname,char(predirection_ranges(sh_idx))); %current direction
...
rowNumber = 2;
for i=1:size(txt2,2)
    range=sprintf('B%d',rowNumber);
    xlswrite(filename3,mat_output{1,i},shname,range); %output model
    nf_inmodel=size(mat_output{1,i},1);
    rowNumber=rowNumber+nf_inmodel+3;
end


xlswrite(filename4,m2xdate(today()),shname,'a1'); %today's date
xlswrite(filename4,current_Metrics,shname,'b3'); %output trades
end
% t_ranges={'a2:v6','a2:v13','a2:v7','a2:v3','a2:v8','a2:v5','a2:v6','a2:v3','a2:v7','a2:v15','a2:v6','a2:v3','a2:v4'};
% data1=readtable(filename4,'sheet',shname,'range','a2:v4');
% for j=1:size(t_ranges,2)
%     data1=readtable(filename4,'sheet',char(v_names(j)),'range',char(t_ranges(j)));
% % [num1,txt1]=xlsread(filename1,shname,char(t_ranges(sh_idx))); %factor
% end
% j=1;
% data1=readtable(filename4,'sheet',char(v_names(j)),'range',char(t_ranges(j)));
