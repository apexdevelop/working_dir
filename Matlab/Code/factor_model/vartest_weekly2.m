...
% correlation  line 203:enter_factor=v_factor(end);
clear all;
cd('C:\Users\ychen\Documents\MATLAB');
javaaddpath('C:\blp\API\APIv3\JavaAPI\v3.8.8.2\lib\blpapi3.jar')
% shipping,utility,steel,coal,solar,insurance,aluminum,auto
filename='factors_weekly.xlsx';
shname='shipping';
[~,txt1]=xlsread(filename,shname,'c1:zz1');  %equity
[~,txt2]=xlsread(filename,shname,'a3:a100'); %factor
[~,txt3]=xlsread(filename,shname,'c2:zz2');  %benchmark
...
for n=1:size(txt2,1) %factor
    ...
    for j=1:v_idx_nemp(end)
        if j<=v_idx_nemp(idx+1)           
           raw_re_rtn_5d(j)=raw_rtn_5d(v_idx_nemp(idx))/v_d(idx);
    ...
    for j=v_idx_nemp(end):size(raw_rtn_5d,1)
        raw_re_rtn_5d(j)=raw_rtn_5d(v_idx_nemp(end))/v_d(end);
