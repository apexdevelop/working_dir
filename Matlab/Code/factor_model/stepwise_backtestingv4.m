...
[direction,~]=xlsread('factors_pval.xlsx',shname,'d3:zz3'); 
[v_TH,~]=xlsread('factors_pval.xlsx',shname,'d4:zz4');

[v_inmodel,~]=xlsread('factors_pval.xlsx',shname,'a6:a100');
    ...
    while j<=size(rtn_Y,1)
          ...
          if sum(inmodel)==0
             ...
             while sum(inmodel)==0 && p_enter(sh_idx)+step<1 && p_exit(sh_idx)+step<1
                   [b,se,pval,inmodel,stats,nextstep,history]=stepwisefit(X,Y,'penter',p_enter(sh_idx)+step,'premove',p_exit(sh_idx)+step,'display','off');   
                   step=step+0.1;
             end
...
%              T_pred=size(rtn_Y,1)-j;
    ...
    current_metric=[current_metric mean(v_whole_rsqr) lt_coint_p zscr(end-5) zscr(end-10) zscr(end-20) zscr(end-30)];
