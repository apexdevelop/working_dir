% Zspread=Zspread_test;
% f_ret=factor_test;
% z_fpx=z_fpx_test;
% enter_fret=enter_factor;
% v_causal=causal_test;
% tday=date_test;
% effect=effect(n,m);
function [s, ret_v,r,output]=factor_backtest3(v_rtn,Zspread,f_ret,z_fpx,enter_fret,v_causal,enter_causal,exit_causal,tday,effect)
%% Calculate Performance statistics based on current spread
exit_ratio=0.25;
trades=0;                          
Enter=[];
Exit=[];
Long=[];
Short=[];
ret_v=[];
ret_2w_v=[];
hp_v=[];
is_open=0;
direction=0;
last_direction=0;
hp_TH=40;
s = zeros(size(f_ret,1),size(f_ret,2));
r_trade = zeros(size(f_ret,1),1);
r = zeros(size(f_ret,1),1); %daily return
CPnL=zeros(size(f_ret,1),1);

for j=2:size(f_ret,1)
         if is_open~=0 %that means position opened  
            s(j,1)=s(j-1,1);
            s(j,2) = - s(j, 1);
            r_trade(entryIDX)=0;
            current_r=sum([0 0; s(entryIDX:j, :) .* v_rtn(entryIDX:j,:)] ,2);
            r_trade(j)= sum(current_r);
            cumul_r= sum([0 0; s(1:j, :) .* v_rtn(1:j,:)] ,2);
            CPnL(j)= sum(cumul_r);
            
            %exit
            if (direction==1 && (effect==1 && z_fpx(j)<enter_fpx-2*exit_ratio || effect==-1 && z_fpx(j)>enter_fpx+2*exit_ratio)) || v_causal(j) > exit_causal || r_trade(j)<-15 || j-entryIDX > hp_TH
                trades=trades+1;
                ret_v=[ret_v;r_trade(j)];
                hp_v=[hp_v;j-entryIDX];
                w_ret=100*((r_trade(j)/100+1)^(10/(j-entryIDX))-1);
                ret_2w_v=[ret_2w_v;w_ret];
                Exit=[Exit;j];
                is_open=0;
                direction=0;
            elseif (direction==-1 && (effect==1 && z_fpx(j)>enter_fpx+2*exit_ratio || effect==-1 && z_fpx(j)<enter_fpx-2*exit_ratio)) || v_causal(j) > exit_causal || r_trade(j)<-15 || j-entryIDX > hp_TH
                trades=trades+1;
                ret_v=[ret_v;r_trade(j)];
                hp_v=[hp_v;j-entryIDX];
                w_ret=100*((r_trade(j)/100+1)^(10/(j-entryIDX))-1);
                ret_2w_v=[ret_2w_v;w_ret];
                Exit=[Exit;j];
                is_open=0;
                direction=0;
            else
            end
            
         else   %position not opened
            CPnL(j)= CPnL(j-1);
            if   (effect==1 && f_ret(j)<= -enter_fret || effect==-1 && f_ret(j)>= enter_fret) && v_causal(j) <= enter_causal ...
                    && Zspread(j)>-0.5
                s(j, 1) = -1;
                s(j,2) = - s(j, 1);
                entryIDX=j;
                is_open=1;
                direction=-1;
                Enter=[Enter;j];
                Short=[Short;j];
                enter_fpx=z_fpx(j);
            elseif  (effect==-1 && f_ret(j)<= -enter_fret || effect==1 && f_ret(j)>= enter_fret) && v_causal(j) <= enter_causal ...
                    && Zspread(j)<0.5
                s(j, 1) = 1;
                s(j,2) = - s(j, 1);
                entryIDX=j;
                is_open=1;
                direction=1;
                Enter=[Enter;j];
                Long=[Long;j];
                enter_fpx=z_fpx(j);
            else
            end
         end
end

if isempty(Enter)==0
   last_enter=m2xdate(tday(Enter(end)),0); 
else
   last_enter=m2xdate(700000,0);
end

if isempty(Enter)==0 && isempty(Exit)==0
      last_exit=m2xdate(tday(Exit(end)),0);
      if isempty(Long)==0 && isempty(Short)==0
         if Long(end)>Short(end)
            last_direction=1;
         else
            last_direction=-1;
         end
      end
      wins=size(find(ret_v>0),1);                       
      av_ret=mean(ret_v);
      av_ret_2w=mean(ret_2w_v);
      exp_vol=std(ret_v);
      exp_vol_ann=std(ret_2w_v);
      hp=round(mean(hp_v));
      winp=wins/trades;
else
      last_exit=m2xdate(700000,0);
      av_ret=-100;
      av_ret_2w=-100;
      exp_vol=-100;
      exp_vol_ann=-100;
      hp=-100;
      winp=-100;
      trades=-100; 
end
% daily return
r  = sum([0 0; s(1:end, :) .* v_rtn] ,2);

%% build output

output=[av_ret,exp_vol,winp,trades,hp,last_exit,last_enter,direction,last_direction,av_ret_2w,exp_vol_ann];