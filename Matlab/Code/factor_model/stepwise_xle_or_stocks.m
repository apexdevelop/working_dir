% line 340 put negative beta factor out of model
clear all;
...
for q=1:size(txt2,2)
    ...
    sum_inmodel=sum(hist_inmodel,1)';
    mat_sum_inmodel=[mat_sum_inmodel sum_inmodel];
    % multi-collinearity test
    inmodel_X=X(:,inmodel);
    R0 = corrcoef(inmodel_X);
    VIF = diag(inv(R0))';
    count_big_VIF=size(find(VIF>5),2);
    mat_big_VIF=[mat_big_VIF count_big_VIF];
    %kappa0 = cond(inmodel_X);
    [sValue,condIdx,VarDecomp] = collintest(inmodel_X,'display','off');
    big_cond=condIdx(end);
    mat_big_cond=[mat_big_cond big_cond];
    
    mat_inmodel=[mat_inmodel transpose(inmodel)];
    mat_b=[mat_b b];
    %% calculating spread
    mat_coint_b=[];
    %exclude negative correlated factor
    coint_inmodel=inmodel;
    coint_inmodel(b<0)=0;
    [~,lt_coint_p,~,~,~] = egcitest(log([Px_Y(:,1) Px(:,coint_inmodel)]),'test','t2');
    j=M;
    while j<=size(rtn_Y,1)
          log_XY1=log([Px_Y(j-M+1:j,1) Px(j-M+1:j,coint_inmodel)]);
          
          if j<=size(rtn_Y,1)-N
             [h,coint_p,~,~,reg] = egcitest(log_XY1,'test','t2');
             coint_b=[1;-reg.coeff(2:end)];
             mat_coint_b=[mat_coint_b coint_b];
             log_XY2=log([Px_Y(j:j+N-1,1) Px(j:j+N-1,coint_inmodel)]);
             ...
             log_XY2=log([Px_Y(j:end,1) Px(j:end,coint_inmodel)]);
