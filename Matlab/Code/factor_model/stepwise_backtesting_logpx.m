...
shname='nol';
[~,txt1]=xlsread('factors.xlsx',shname,'b5:b28'); %factor
[~,txt2]=xlsread('factors.xlsx',shname,'d1:zz1'); %equity
% [~,txt2]=xlsread('factors.xlsx',shname,'e1:e1'); %equity
...
%% generate Equity and factor's Data
...
for q=1:size(txt2,2)
    ...
    v_whole_rsqr=[];
    while j<=size(Px_Y,1)
          X=Px_Y(j-M+1:j,2:end);
          Y=Px_Y(j-M+1:j,1);
          [b,se,pval,inmodel,stats,nextstep,history]=stepwisefit(X,Y,'penter',0.01,'premove',0.10,'display','off');
          
          temp_quasi_rsqr=[];
          for j1=1:M
              new_quasi_rsqr=[];
              p_temp=[Px_Y(j-M+j1,2:end)'.*(inmodel'.*b);stats.intercept];
              if Px_Y(j-M+j1,1)~=0
                 new_quasi_rsqr=p_temp./Px_Y(j-M+j1,1);
              end
          ...
          if j<=size(rtn_Y,1)-N         
             temp=Px_Y(j:j+N,2:end)*(inmodel'.*b)+stats.intercept;         
             Ypred(j:j+N,1)=temp;
          else
             T_pred=size(rtn_Y,1)-j;
             temp=Px_Y(j:end,2:end)*(inmodel'.*b)+stats.intercept;
             Ypred(j:end,1)=temp; 
          end
          j=j+N;
    end
