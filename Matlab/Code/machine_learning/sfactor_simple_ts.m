...
%% Calculate inputs        
        ...
        ex_mov_rtn=transpose(er_mov(1:nob-M)-br_mov(1:nob-M));
        %nn matlab, observations are by column
        rowX=transpose(input_fpx);
        rowT=transpose(ex_mov_rtn);
        X=num2cell(rowX);
        T=num2cell(rowT);
        %% nn training
...
%         date_backtest=date(N+1:end);
%         y_TH=prctile(Y,TH_prc);
%         ex_rtn=ex_fullrtn(N+1:end);
