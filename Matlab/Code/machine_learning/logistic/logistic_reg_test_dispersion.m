...
shname='nky_sell2';
[data,~]=xlsread(filename,shname,'c18:j5000'); %factor
lag=0;
X = data(1:end-lag, 1:6); y = data(lag+1:end, 8);
...
[data_test,~]=xlsread(filename,shname,'c3:h17'); %factor
X_test = [ones(size(data_test,1), 1) data_test];
p1_test= predict(theta1, X_test);
%% Logistic Regression
...
X2 = mapFeature4D(X_map(:,1), X_map(:,2), X_map(:,3), X_map(:,6),5);
...
X_map_test=data_test;
X2_test = mapFeature4D(X_map_test(:,1), X_map_test(:,2), X_map_test(:,3), X_map_test(:,4),5);
p2_test = predict(theta2, X2_test);