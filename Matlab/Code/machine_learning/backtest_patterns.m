...
function[rebalance_current_day,rebalance_test_next_ret,rebalance_train_ret,rebalance_train_max_idx,rebalance_output]=backtest_patterns(rtn_Y,disp_col,z_px,z_turnover_mov,z_disp_mov,tday1,M,N,l_pattern,l_np,prc,weights)
%% Calculate inputs
% X=z_px;
% Y=z_turnover_mov;
% Z=z_disp_mov;
...
%       [~,next_max_px_idx]=max(z_px(t1+l_pattern+1:t1+l_pattern+l_np,1));
%       [~,next_min_px_idx]=min(z_px(t1+l_pattern+1:t1+l_pattern+l_np,1));
      v_train_next_max(count_group)=next_max_ret;
      v_train_next_min(count_group)=next_min_ret;
      v_train_next_max_idx(count_group)=next_max_ret_idx;
      v_train_next_min_idx(count_group)=next_min_ret_idx;
...
%   %classify max by return
%   upside_np_max=fit_np_max(fit_np_ret>0);
%   downside_np_max=fit_np_max(fit_np_ret<=0);
%   
...
%   disp_small_volume=std(small_np_volume);
  ...
  mat_fit=[fit_np_ret' fit_np_max_idx'];
  pd=pdist(mat_fit);
  sum_pd=sum(pd);
  [b,~,~,~,stats] = regress(fit_np_max_idx',[ones(size(fit_np_ret,2),1) fit_np_ret']);
...
%   rebalance_small_output=[rebalance_small_output;new_small_output];
  ...
  rebalance_train_max=[rebalance_train_max fit_np_max'];
  rebalance_train_max_idx=[rebalance_train_max_idx fit_np_max_idx'];
