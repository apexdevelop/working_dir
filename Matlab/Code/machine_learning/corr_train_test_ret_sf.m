...
% weights=[0.3;0.3;0.3;0.3;0.3;0.1];
function accuracy=corr_train_test_ret_sf(vYret,vXret,vDate,M,N,l_pattern,l_np,prc,w1,w2,w3,w4,w5,w6)
% Calculate inputs
...
%       [next_max_ret,next_max_ret_idx]=max(vYret(t1+l_pattern+1:t1+l_pattern+l_np,1));
%       [next_min_ret,next_min_ret_idx]=min(vYret(t1+l_pattern+1:t1+l_pattern+l_np,1));
...
%   if size(TH_idx,2)>n_group*prc/100
%      TH_idx=TH_idx(1:n_group*prc/100);
%   end
...
% accuracy=corr(rebalance_exp_ret,rebalance_test_next_ret);
