...
%     disp_row=tsmovavg(disp_row,'e',mov_window);
    ...
    for j=1:(itr_exit_disp+1)
        ...
        [~,~,~,newmetric]=disp_backtest_corr(rtn_Y(corr_window:end,1),z_disp_mov(corr_window:end),enter_disp,exit_disp,z_corr,enter_corr,exit_corr,tday1(corr_window:end)); 
...
%         for p=1:(itr_exit_corr+1)
        for p=1:1
            ...
            if newmetric_corr(7)>=TH_ntrades
               count_feasible_corr=count_feasible_corr+1;
               ...
               clong{count_feasible_corr,1}=newlong;
               cshort{count_feasible_corr,1}=newshort;
               cexit{count_feasible_corr,1}=newexit;
            end
        end
    end
    % find optimal point and Output  
    if count_feasible_corr>=1
%         optiV_corr=v_Omega_corr;
        optiV_corr=v_WL_corr;
    ...
    for j=1:(itr_enter_disp+1)
        ...
        for p=1:(itr_exit_disp+1)
            ...
            [~,~,~,newmetric_gl]=disp_backtest_corr(rtn_Y(corr_window:end,1),z_disp_mov(corr_window:end),enter_disp_gl,exit_disp_gl,z_corr,enter_corr,exit_corr,tday1(corr_window:end)); 
