...
%         temp_rtn=temp_rtn(IX(n_top_vol+1:end-n_top_vol+1)); %keep middle
    ...
    M=corr_window;
    v_hurst=zeros(size(rtn_Y,1)-M+1,1);
    for j=M:size(rtn_Y,1)
        temp_diffrtn=rtn_Y(j-M+1:j,1);
        v_hurst(j-M+1)=hurst_exponent(temp_diffrtn');
    end
...
%     last_disp=z_disp_mov(end);
    ...
    for i1=1:(itr_enter+1)
        ...
        [s,newmetric]=backtest_hurst(rtn_Y(corr_window:end,1),z_disp_mov(corr_window:end),v_hurst,enter_disp,exit_disp,tday1); 
        if newmetric(5)>=4
