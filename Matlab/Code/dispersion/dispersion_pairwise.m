...
[num,txt]=xlsread('dispersion matlab','Universe','f1:f1000');
...
%     px1(isnan(px1))=[];
    ...
    %% Calculate pairwise Dispersion
    n_pair = n_element*(n_element-1)/2;
    v_disp=zeros(size(rtn_Y,1),1);
    v_disp_mov=zeros(size(rtn_Y,1),1);
    v_disp_5d=zeros(size(rtn_Y,1),1);
    
    for j=corr_window:size(rtn_Y,1)
        %v_drtn=rtn_Y(j,2:end); %%daily return array
        %v_drtn(isnan(v_drtn))=[];
        temp_disp = 0;
        for p = 1:(n_element-1)
            rtn1 = rtn_Y(j-corr_window+1:j,p+1);
            for q = (p+1):n_element
                rtn2=rtn_Y(j-corr_window+1:j,q+1);
                if (~isnan(corr(rtn1,rtn2)))
                    temp_disp = temp_disp + corr(rtn1,rtn2);
                end
            end
        end
        v_disp(j)=temp_disp/n_pair;
    end
...
%     v_disp=mean(v_diff,2);
    ...
    for j=1:size(rtn_Y,1)
        temp_rtn1=rtn_5dY(j,2:end);
        temp_rtn1(isnan(temp_rtn1))=[];
        temp_avg=mean(temp_rtn1);
        temp_diff=abs(temp_rtn1-repmat(temp_avg,1,size(temp_rtn1,2)));
...
xlswrite('dispersion matlab',[excel_date disp_col excel_date z_disp_mov excel_date v_disp excel_date z_disp excel_date v_disp_5d excel_date z_disp_5d],'pairwise','a2');