function [Long_longvector,Short_longvector,Exit_longvector,output]=disp_backtest_nocorr(v_rtn,Y,enter_disp,exit_disp,tday)
...
if enter_disp>0
     for j=2:size(Y,1)
         if is_open~=0 %that means position opened  
            ...
            %exit
            if (direction==1 && Y(j)<exit_disp + 0.05) || r_trade(j)<pnL_TH || j-entryIDX > hp_TH
                trades=trades+1;
...
output=[enter_disp,exit_disp,av_ret,stdev,trades,hp,omega,winp,last_exit,last_enter];