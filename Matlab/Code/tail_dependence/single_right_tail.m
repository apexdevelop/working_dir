...
% txt1={'TUR US Equity';'6301 JP Equity'};
% txt2={'SPX Index';'TPX Index'};
% javaaddpath('C:\blp\API\APIv3\JavaAPI\v3.6.1.0\lib\blpapi3.jar')
% txt3={'09/05/2012';'Equity';200;20};

function [v_tday,M_rolling,v_lastCorr,Mat_meanIC]=single_right_tail(txt1,txt2,txt3)
%% GET Bloomberg DATA
...
for i=1:size(v_lookback,2)
    ...
    for lag=1:nlags
        ...
        v_R1=[];
        while j<=s1
          R1=yan_right_tail(factor1(j-M_varied+1:j),factor2(j-M_varied+1:j),lag);
          j=j+N;
          v_R1=[v_R1;R1];
        end
