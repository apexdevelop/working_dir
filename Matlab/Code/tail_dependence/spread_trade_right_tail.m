...
% txt1={'CCN+12M Curncy';'2628 HK Equity'};
% txt2={'CCN+1M Curncy';'HSCEI Index'};
% javaaddpath('C:\blp\API\APIv3\JavaAPI\v3.7.1.1\lib\blpapi3.jar')
% txt3={'05/04/2012'};


function [M_corr,M_PnL,M_lag1,M_lag2]=spread_trade_right_tail(txt1,txt2,txt3)
%% GET Bloomberg DATA
...
%     rel_Rtn2=extract_country_single(rtn(px2),rtn(px_f2),1,1); %return
%     factor2=[0;rel_Rtn2];

    factor2=extract_country_single([px2 px_f2],3); %price
...
%           R1=yan_right_tail(factor1(j-M+1:j),factor2(j-M+1:j),lag);
          R1=right_tail(factor1(j-M+1:j),factor2(j-M+1:j),lag);
          if j<=s1-N
