function output=factor_backtest(v_rtn,Zspread,f_ret,enter_fret,v_causal,enter_causal,exit_causal,tday)
...
for j=2:size(f_ret,1)
         if is_open~=0 %that means position opened  
            ...
            %exit
            if (direction==1 && f_ret(j)<-enter_fret) || v_causal(j) > exit_causal || r_trade(j)<-15 || j-entryIDX > hp_TH
                trades=trades+1;
                ...
                is_open=0;
                direction=0;
            elseif (direction==-1 && f_ret(j)>enter_fret) || v_causal(j) > exit_causal || r_trade(j)<-15 || j-entryIDX > hp_TH
                trades=trades+1;
                ret_v=[ret_v;r_trade(j)];
                hp_v=[hp_v;j-entryIDX];
                Exit=[Exit;j];
            ...
            CPnL(j)= CPnL(j-1);
            if   f_ret(j)<= -enter_fret  && v_causal(j) <= enter_causal ...
%                     && Zspread(j)>0.5
                s(j, 1) = -1;
                s(j,2) = - s(j, 1);
                entryIDX=j;
                ...
                Enter=[Enter;j];
                Short=[Short;j];
            elseif  f_ret(j)>= enter_fret && v_causal(j) <= enter_causal ...
%                     && Zspread(j)<-0.5
                s(j, 1) = 1;
                s(j,2) = - s(j, 1);
                entryIDX=j;
