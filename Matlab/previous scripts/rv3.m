x=cbkx(end-250:end,1:2);
a=cbkximp(end-250:end,1:2)/100;
clc;
...
for i=startpoint:size(bet)
    ...
    variance2(i,3)=max(variance2(i-startpoint+boundregress:i,1));
    variance2(i,4)=min(variance2(i-startpoint+boundregress:i,1));
...
%realized volgain
subplot(3,3,1), plot(bet(startpoint:end-impterm,1:4)), subplot(3,3,2),plot(idsy(startpoint:end-impterm,1:3))  
subplot(3,3,3),plot(trades2(startpoint:end,6)), subplot(3,3,4),plot(trades2(startpoint:end,7)) 
subplot(3,3,5), plot(volgain(startpoint:end,3)), subplot(3,3,6),plot(trades2(startpoint:end,3))
