...
for j=1:nsets;
 ...
 if rnum == 1;fprintf(fid,'%5s ',dstr);     
  ...
  if i <= endc
% find version #; 
 %Punit [version,junk] = version; vers = str2num(version);
  %Punit if vers == 5.2
   %Punit fprintf(fid,sfmt,strjust(cnames(i,:),'right'));
  % else
...
% find version #; 
 %Punit [version,junk] = version; vers = str2num(version);
 %Punit  if vers == 5.2
   %Punitfprintf(fid,sfmtv{i},strjust(cnames(i,:),'right'));
   %Punitelse
