...
tday1=txt(start+4:last+4, i*2-1); % the first column (starting from the second row) is the trading days in format mm/dd/yyyy.
adjcls1=num(start:last, i*2-1); %ppp the last column contains the adjusted close prices.


for j=i+1:y
 if (clu1(i,1)==clu1(j,1))
tday2=txt(start+4:last+4, j*2-1); % the first column (starting from the second row) is the trading days in format mm/dd/yyyy.
adjcls2=num(start:last, j*2-1); % PPP
